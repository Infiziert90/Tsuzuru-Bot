...
Differences:
 ...
 - changed the cubic argument to descale_getnative_kernel,
...
"""
def descale_getnativeM(src, w, h, thr=None, expand=None, inflate=None, descale_getnative_kernel=None, kernel=None, kernely=None, kerneluv=None,
    ...
    kwargsdict = {'src': [src, (vs.VideoNode,)], 'w': [w, (int,)], 'h': [h, (int,)], 'thr': [thr, (int,)],
                  'expand': [expand, (int,)], 'inflate': [inflate, (int,)],'descale_getnative_kernel': [descale_getnative_kernel, (str,)],
    ...
    for k, v in kwargsdict.items():
        if v[0] is not None and not isinstance(v[0], v[1]):
            raise TypeError('descale_getnativeM: "{variable}" must be {types}!'
    ...
    if showmask is None:
        showmask = 0
    if descale_getnative_kernel is None:
        descale_getnative_kernel = 'bilinear'
    elif descale_getnative_kernel.lower().startswith('de'):
        descale_getnative_kernel = descale_getnative_kernel[2:]
    if kernely is None:
        kernely = kernel
    if kerneluv is None:
        kerneluv = kernel
    ...
    dbi = Resize(src_y, w, h, kernel=descale_getnative_kernel, a1=b, a2=c, taps=taps, invks=True)
    dbi2 = Resize(dbi, ow, oh, kernel=descale_getnative_kernel, a1=b, a2=c, taps=taps)
...
# descale_getnativeM alias
DebilinearM = partial(descale_getnativeM, descale_getnative_kernel='bilinear')

# descale_getnativeM alias
DebicubicM = partial(descale_getnativeM, descale_getnative_kernel='bicubic')

# descale_getnativeM alias
DelanczosM = partial(descale_getnativeM, descale_getnative_kernel='lanczos')

# descale_getnativeM alias
Despline16M = partial(descale_getnativeM, descale_getnative_kernel='spline16')

# descale_getnativeM alias
Despline36M = partial(descale_getnativeM, descale_getnative_kernel='spline36')
...
"""
def descale_getnativeAA(src, w=1280, h=720, thr=10, kernel='bilinear', b=1/3, c=1/3, taps=3,
...
# Legacy descale_getnativeAA alias
def ProtectedDebiXAA(src, w=1280, h=720, thr=10, expand=3, inflate=3,
                     bicubic=False, b=1/3, c=1/3, showmask=False, bits=None):

    if bicubic:
        return descale_getnativeAA(src, w=w, h=h, thr=thr, kernel='bicubic', b=b, c=c, taps=None,
                         expand=expand, inflate=inflate, showmask=showmask)
    else:
        return descale_getnativeAA(src, w=w, h=h, thr=thr, kernel='bilinear', b=None, c=None, taps=None,
                         expand=expand, inflate=inflate, showmask=showmask)
